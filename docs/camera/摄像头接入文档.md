...
> **文档说明**：本文档整合了完整的摄像头调试过程，包含所有终端输出和报错信息。可替代 `log.md` 和 `test_log.md`。

## 目录
- [1. 问题背景](#1-问题背景)
- [2. OV5695 摄像头接入失败](#2-ov5695-摄像头接入失败)
- [3. USB 摄像头接入](#3-usb-摄像头接入)
- [4. 测试与验证](#4-测试与验证)
- [5. 故障排查总结](#5-故障排查总结)
...
本项目最初计划使用 **OV5695 MIPI-CSI 摄像头** 通过 RK3568 的硬件 ISP 接口实现人脸识别。但在实际部署过程中遇到了硬件故障，最终切换到 **USB 摄像头**方案。

**调试时间**：2025-12-25

本文档记录：
- ✅ 完整的排查过程（设备树、I2C、GPIO、驱动）
- ✅ 所有报错信息（dmesg、i2cdetect、v4l2-ctl）
- ✅ USB摄像头切换方案
- ✅ 性能测试结果（OpenCV 30 FPS）
...
**问题描述**：
- OV5695 摄像头通过 MIPI 排线连接到 RK3568
- 系统能检测到设备节点 `/dev/video0`，但无法正常工作
...
**错误分析**：
- 驱动尝试通过 I2C 读取传感器 ID
- 返回值为 `000000`（应该返回 OV5695 的芯片 ID）
- 错误代码 `-5` 表示 I/O 错误
...
**输出**：
```
crw-rw---- 1 root i2c 89, 0 12月 25 07:58 /dev/i2c-0
crw-rw---- 1 root i2c 89, 1 12月 25 07:58 /dev/i2c-1
crw-rw---- 1 root i2c 89, 2 12月 25 07:58 /dev/i2c-2
crw-rw---- 1 root i2c 89, 4 12月 25 07:58 /dev/i2c-4
crw-rw---- 1 root i2c 89, 5 12月 25 07:58 /dev/i2c-5
crw-rw---- 1 root i2c 89, 6 12月 25 07:58 /dev/i2c-6
```

#### 扫描 I2C-2（摄像头应该在这个总线上）
...
🔴 **致命问题**：地址 `0x36` 位置显示 `--`，表示 **I2C 总线上没有检测到设备**！
...
**结论**：在所有 I2C 总线上都**没有检测到地址 0x36 的设备**。
...
✅ **摄像头电源 GPIO（gpio-17）状态为 HIGH（已供电）**

#### 查看 reset 和 pwdn GPIO
...
🔴 **驱动模块不存在或已内置到内核中**（内置则不需要 modprobe）
...
**输出**：
```
crw-rw----+ 1 root video 81, 0 12月 25 07:58 /dev/video0
crw-rw----+ 1 root video 81, 1 12月 25 07:58 /dev/video1
crw-rw----+ 1 root video 81, 2 12月 25 07:58 /dev/video2
crw-rw----+ 1 root video 81, 3 12月 25 07:58 /dev/video3
crw-rw----+ 1 root video 81, 4 12月 25 07:58 /dev/video4
crw-rw----+ 1 root video 81, 5 12月 25 07:58 /dev/video5
crw-rw----+ 1 root video 81, 6 12月 25 07:58 /dev/video6
crw-rw----+ 1 root video 81, 7 12月 25 07:58 /dev/video7
crw-rw----+ 1 root video 81, 8 12月 25 07:58 /dev/video8
lrwxrwxrwx  1 root root      6 12月 25 07:58 /dev/video-camera0 -> video0
```

✅ **video 设备节点存在**（这些是 ISP 管道节点，不是摄像头传感器）
...
| 设备状态 | ✅ okay | 驱动已启用 |
| I2C 地址配置 | ✅ 0x36 | 配置正确 |
| 驱动加载 | ⚠️ 加载但检测失败 | 驱动工作但无法通信 |
| I2C 扫描 | ❌ 未检测到设备 | **硬件无响应** |
| GPIO 电源 | ✅ 已供电 | 电源控制正常 |
| 传感器 ID | ❌ 返回 000000 | **I2C 通信失败** |

**最终结论**：
```
🔴 OV5695 摄像头硬件故障
   - I2C 总线无法检测到设备（地址 0x36 无响应）
   - 可能原因：摄像头模块损坏、排线接触不良、MIPI 接口问题
   - 解决方案：更换摄像头或切换到 USB 摄像头
```
...
🔴 **问题**：该USB接口可能供电不足或硬件故障

---

### 3.3 更换 USB 接口

**操作**：将摄像头更换到另一个 USB 2.0 接口
...
✅ **USB CAMERA 已识别（Bus 002 Device 003）**
...
✅ **摄像头映射到 /dev/video9 和 /dev/video10**

**说明**：
- `/dev/video9`：视频流节点（主要使用）
- `/dev/video10`：元数据节点
...
============================================================
  OV5695 摄像头测试
============================================================
测试时间: 2025-12-25 08:56:55
目标分辨率: 640x480
输出目录: /home/topeet/face_detection_system/camera_test_output

============================================================
  1. 检查 Video 设备
============================================================
找到 12 个 video 设备：
  /dev/video-camera0: 跳过（符号链接或非标准设备）
...
[✓ PASS] 图像质量检查
      亮度正常
[✓ PASS] 保存图像
      文件: opencv_test_20251225_085656_640x480.jpg
...
[✓ PASS] 性能测试
      FPS 良好 (30.11)
保存最后一帧: continuous_test_20251225_085702_640x480.jpg
...
============================================================
  建议
============================================================
✓ OpenCV 模式工作正常，建议使用此模式
  配置: CAMERA_MODE = 'opencv' 或 'auto'
```
...
[✓ PASS] 性能测试
      FPS 良好 (30.11)
保存最后一帧: continuous_test_20251225_090317_640x480.jpg
...
**原因分析**：
- USB 摄像头不支持 `rkisp` 插件（rkisp 是 RK3568 ISP 专用的）
- USB 摄像头使用 UVC 协议，应该用标准的 `v4l2src` 插件
- 但对于门禁系统，OpenCV 模式的 30 FPS 已经足够
...
```python
# 摄像头配置
CAMERA_MODE = 'opencv'  # USB 摄像头使用 OpenCV 模式
CAMERA_INDEX = 9        # USB CAMERA 在 /dev/video9
CAMERA_WIDTH = 640
CAMERA_HEIGHT = 480
CAMERA_FPS = 30
```
...
| 故障现象 | 排查结果 | 结论 |
|---------|---------|------|
| 驱动报错：sensor id(000000) | I2C 通信失败 | 硬件无响应 |
| i2cdetect 未检测到 0x36 | I2C 总线扫描失败 | 物理连接问题 |
| GPIO 电源正常 | vcc_camera = HIGH | 电源控制正常 |
| 设备树配置正常 | status = okay | 软件配置正确 |
| **最终判定** | **硬件故障** | **摄像头损坏或排线问题** |
...
| 步骤 | 要点 | 常见问题 |
|------|------|---------|
| 1. 插入硬件 | 使用 USB 2.0/3.0 接口 | 某些接口可能无电 |
| 2. 确认识别 | `dmesg -w` 实时监控 | 看到 `uvcvideo: Found UVC` |
| 3. 查找节点 | `v4l2-ctl --list-devices` | 记录 /dev/videoX |
| 4. 测试 | `python3 test_camera.py --index X` | 使用 OpenCV 模式 |
| 5. 配置 | `CAMERA_MODE = 'opencv'` | USB 不支持 GStreamer rkisp |
...
摄像头无法工作
    ↓
检查 dmesg | grep ov5695
    ├─ 有 "Unexpected sensor id(000000)"
    │    ↓
    │  运行 i2cdetect -y 2
    │    ├─ 地址 0x36 显示 --
    │    │    ↓
...
**`i2cdetect` 是硬件问题的金标准**：
- 如果扫描到设备（显示地址或 UU）→ 硬件正常，排查软件
- 如果扫描不到（显示 --）→ 硬件问题，检查连接或更换

### 6.3 USB 摄像头的优势

| 对比项 | MIPI 摄像头（OV5695） | USB 摄像头 |
|-------|---------------------|-----------|
| 接口 | MIPI-CSI | USB 2.0/3.0 |
| 驱动 | 需要设备树配置 | 免驱（UVC） |
| 调试难度 | 高（涉及 I2C、GPIO、ISP） | 低（即插即用） |
| 性能 | 高（硬件 ISP） | 中（CPU 软解） |
| 故障率 | 高（排线、接口易损） | 低（标准接口） |
| 适用场景 | 嵌入式产品 | 快速原型、开发调试 |
...
- V4L2 API 参考
- 项目文档：
  - `docs/RK3568开发板配置指南.md`
  - `docs/backend/ov5695接入文档.md`

---

**结论**：通过系统性的硬件排查，确定 OV5695 摄像头硬件故障，成功切换到 USB 摄像头方案，实现了稳定的 30 FPS 视频采集。
