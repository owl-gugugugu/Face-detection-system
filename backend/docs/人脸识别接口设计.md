# 人脸识别接口设计文档

本文档定义后端（FastAPI）与人脸识别模块（C++ .so 库）的封装接口规范。

---

## 1. 分层架构

采用三层架构，清晰分离职责：

```
┌──────────────────────────────────────┐
│   Layer 3: FastAPI 路由              │
│   - 接收 HTTP 请求                    │
│   - 调用 FaceEngine                  │
│   - 数据库查询/存储                   │
│   - 返回 JSON 响应                    │
└──────────────────────────────────────┘
                 ↓ 调用
┌──────────────────────────────────────┐
│   Layer 2: Python Wrapper            │
│   (backend/core/face_engine.py)     │
│   - 加载 .so 动态库                   │
│   - 管理 ctypes 内存指针              │
│   - 提供 Python 友好接口              │
│   - 输入: bytes → 输出: List[float]   │
└──────────────────────────────────────┘
                 ↓ ctypes
┌──────────────────────────────────────┐
│   Layer 1: C++ 动态库                │
│   (libface_engine.so)               │
│   - 人脸检测 (RetinaFace)            │
│   - 人脸对齐 (仿射变换)               │
│   - 特征提取 (MobileFaceNet)         │
│   - 输出: 512维特征向量               │
└──────────────────────────────────────┘
```

**职责划分**：
- **Layer 1 (C++)**: 纯计算，负责图像处理和模型推理
- **Layer 2 (Python Wrapper)**: 纯翻译，负责 Python ↔ C++ 数据转换
- **Layer 3 (FastAPI)**: 业务逻辑，负责数据库、权限、HTTP 响应

**重要原则**：Wrapper 层**不应包含**任何业务逻辑（数据库、权限、开门控制等）。

---

## 2. Python Wrapper 实现规范

### 2.1 文件位置

```
backend/core/face_engine.py
```

### 2.2 必须实现的 5 个核心部分

#### Part 1: 动态库加载与路径管理

**功能**：
- 定位 `libface_engine.so` 和两个 RKNN 模型文件
- 使用 `ctypes.CDLL` 加载动态库
- 检查文件是否存在，提供清晰的错误提示

**实现示例**：
```python
backend_dir = Path(__file__).parent.parent.resolve()
project_root = backend_dir.parent

lib_path = project_root / "face_detection" / "build" / "libface_engine.so"
retinaface_model = project_root / "face_detection" / "models" / "RetinaFace.rknn"
mobilefacenet_model = project_root / "face_detection" / "models" / "mobilefacenet.rknn"

# 检查文件存在性
if not lib_path.exists():
    raise FileNotFoundError(f"Library not found: {lib_path}")

# 加载动态库
self.lib = ctypes.CDLL(str(lib_path))
```

**关键点**：使用绝对路径，避免相对路径导致的部署问题。

---

#### Part 2: C 函数的参数类型定义

**功能**：告诉 Python 每个 C 函数的参数类型和返回值类型。**这是最关键的步骤，配置错误会导致程序崩溃。**

**必须定义的 5 个 C 函数**：

| C 函数 | 参数类型 | 返回值类型 |
|--------|---------|-----------|
| `FaceEngine_Create` | 无参数 | `c_void_p` (引擎指针) |
| `FaceEngine_Init` | `c_void_p`, `c_char_p`, `c_char_p` | `c_int` (状态码) |
| `FaceEngine_ExtractFeature` | `c_void_p`, `POINTER(c_ubyte)`, `c_int`, `POINTER(c_float)` | `c_int` (状态码) |
| `FaceEngine_Destroy` | `c_void_p` | `None` |
| `FaceEngine_CosineSimilarity` | `POINTER(c_float)`, `POINTER(c_float)` | `c_float` (相似度) |

**实现示例**：
```python
# void* FaceEngine_Create()
self.lib.FaceEngine_Create.restype = ctypes.c_void_p
self.lib.FaceEngine_Create.argtypes = []

# int FaceEngine_Init(void* engine, const char* model1, const char* model2)
self.lib.FaceEngine_Init.restype = ctypes.c_int
self.lib.FaceEngine_Init.argtypes = [
    ctypes.c_void_p,     # engine
    ctypes.c_char_p,     # retinaface_model
    ctypes.c_char_p      # mobilefacenet_model
]

# int FaceEngine_ExtractFeature(void* engine, unsigned char* data, int len, float* out)
self.lib.FaceEngine_ExtractFeature.restype = ctypes.c_int
self.lib.FaceEngine_ExtractFeature.argtypes = [
    ctypes.c_void_p,                   # engine
    ctypes.POINTER(ctypes.c_ubyte),    # jpeg_data
    ctypes.c_int,                      # data_len
    ctypes.POINTER(ctypes.c_float)     # feature_512
]

# void FaceEngine_Destroy(void* engine)
self.lib.FaceEngine_Destroy.restype = None
self.lib.FaceEngine_Destroy.argtypes = [ctypes.c_void_p]

# float FaceEngine_CosineSimilarity(const float* emb1, const float* emb2)
self.lib.FaceEngine_CosineSimilarity.restype = ctypes.c_float
self.lib.FaceEngine_CosineSimilarity.argtypes = [
    ctypes.POINTER(ctypes.c_float),    # emb1
    ctypes.POINTER(ctypes.c_float)     # emb2
]
```

---

#### Part 3: 初始化方法 (__init__)

**功能**：
- 创建 C++ 引擎实例
- 加载两个 RKNN 模型
- 保存引擎指针供后续调用使用

**流程**：
```python
def __init__(self):
    # 1. 加载动态库（Part 1）
    # 2. 定义函数类型（Part 2）

    # 3. 创建引擎实例
    self.engine_ptr = self.lib.FaceEngine_Create()
    if not self.engine_ptr:
        raise RuntimeError("Failed to create FaceEngine instance")

    # 4. 初始化模型
    ret = self.lib.FaceEngine_Init(
        self.engine_ptr,
        str(retinaface_model).encode('utf-8'),
        str(mobilefacenet_model).encode('utf-8')
    )

    if ret != 0:
        self.lib.FaceEngine_Destroy(self.engine_ptr)
        raise RuntimeError(f"Failed to initialize FaceEngine (ret={ret})")
```

**关键点**：
- `engine_ptr` 必须保存为实例变量，后续所有操作都需要它
- 模型路径必须编码为 UTF-8 字节串
- 初始化失败时要释放已创建的引擎

---

#### Part 4: 核心功能方法 (extract_feature)

**功能**：FastAPI 与人脸识别模块的**唯一交互点**。

**接口定义**：
```python
def extract_feature(self, image_bytes: bytes) -> Optional[List[float]]:
    """
    提取人脸特征向量

    Args:
        image_bytes: 图片的二进制数据（JPEG/PNG格式）

    Returns:
        List[float]: 512维特征向量（成功）
        None: 未检测到人脸或处理失败
    """
```

**实现流程**：
```python
# 1. 转换为 ctypes 数组（零拷贝）
jpeg_array = np.frombuffer(image_bytes, dtype=np.uint8)
jpeg_ptr = jpeg_array.ctypes.data_as(ctypes.POINTER(ctypes.c_ubyte))

# 2. 预分配输出缓冲区
feature_512 = np.zeros(512, dtype=np.float32)
feature_ptr = feature_512.ctypes.data_as(ctypes.POINTER(ctypes.c_float))

# 3. 调用 C++ 函数
ret = self.lib.FaceEngine_ExtractFeature(
    self.engine_ptr,
    jpeg_ptr,
    len(image_bytes),
    feature_ptr
)

# 4. 处理返回值
if ret == 0:
    return feature_512.tolist()  # 成功
elif ret == -1:
    return None  # 未检测到人脸
else:
    return None  # 其他错误
```

**返回值说明**：
| C++ 返回值 | 含义 | Python 返回 |
|-----------|------|------------|
| `0` | 成功提取特征 | `List[float]` (512维) |
| `-1` | 未检测到人脸 | `None` |
| `-2` | 图像解码失败 | `None` |
| 其他 | 模型推理失败 | `None` |

---

#### Part 5: 资源释放方法 (__del__)

**功能**：防止内存泄漏，释放 NPU 上下文和内存。

**实现**：
```python
def __del__(self):
    """释放资源（防止内存泄漏）"""
    if hasattr(self, 'engine_ptr') and self.engine_ptr:
        self.lib.FaceEngine_Destroy(self.engine_ptr)
        print("[FaceEngine] Engine destroyed")
```

**关键点**：
- Python GC 调用 `__del__` 时自动释放 C++ 资源
- 必须检查 `engine_ptr` 是否存在（避免初始化失败时报错）

---

### 2.3 额外功能方法 (可选)

#### compute_similarity - 计算余弦相似度

**功能**：计算两个特征向量的相似度（可选，也可在 FastAPI 层用 numpy 实现）。

**接口定义**：
```python
def compute_similarity(self, feature1: List[float], feature2: List[float]) -> float:
    """
    计算两个特征向量的余弦相似度

    Args:
        feature1: 512维特征向量1
        feature2: 512维特征向量2

    Returns:
        float: 余弦相似度 [0, 1]
    """
```

**实现**：
```python
# 转换为 numpy 数组
arr1 = np.array(feature1, dtype=np.float32)
arr2 = np.array(feature2, dtype=np.float32)

# 转换为 ctypes 指针
ptr1 = arr1.ctypes.data_as(ctypes.POINTER(ctypes.c_float))
ptr2 = arr2.ctypes.data_as(ctypes.POINTER(ctypes.c_float))

# 调用 C++ 函数
similarity = self.lib.FaceEngine_CosineSimilarity(ptr1, ptr2)
return float(similarity)
```

---

### 2.4 单例模式实现

**推荐实现**：全局只创建一个引擎实例，避免重复加载模型。

```python
class FaceEngine:
    _instance = None
    _initialized = False

    def __new__(cls):
        """单例模式：确保全局只有一个引擎实例"""
        if cls._instance is None:
            cls._instance = super().__new__(cls)
        return cls._instance

    def __init__(self):
        if self._initialized:
            return
        # ... 初始化逻辑 ...
        self._initialized = True


# 全局单例获取函数（供 FastAPI 使用）
_face_engine_instance = None

def get_face_engine() -> FaceEngine:
    """获取全局 FaceEngine 实例"""
    global _face_engine_instance
    if _face_engine_instance is None:
        _face_engine_instance = FaceEngine()
    return _face_engine_instance
```

---

## 3. FastAPI 接口设计

### 3.1 核心接口

#### 接口 1: 录入人脸

**路由**: `POST /api/face/register`

**功能**：提取人脸特征并存入数据库。

**流程**：
```python
from fastapi import UploadFile
from backend.core.face_engine import get_face_engine

@app.post("/api/face/register")
async def register_face(file: UploadFile, name: str):
    # 1. 读取图片
    image_bytes = await file.read()

    # 2. 提取特征
    engine = get_face_engine()
    feature = engine.extract_feature(image_bytes)

    if not feature:
        return {"code": 400, "message": "未检测到人脸"}

    # 3. 存入数据库
    db.save_face(name=name, feature=feature)

    return {"code": 200, "message": "录入成功"}
```

---

#### 接口 2: 人脸识别

**路由**: `POST /api/face/recognize`

**功能**：识别图片中的人脸（用于测试，实际部署时由开发板后台线程处理）。

**流程**：
```python
@app.post("/api/face/recognize")
async def recognize_face(file: UploadFile):
    # 1. 读取图片
    image_bytes = await file.read()

    # 2. 提取特征
    engine = get_face_engine()
    feature = engine.extract_feature(image_bytes)

    if not feature:
        return {"code": 400, "message": "未检测到人脸"}

    # 3. 查询数据库，计算相似度
    all_faces = db.get_all_faces()  # [(name, feature), ...]

    max_similarity = 0.0
    matched_name = None

    for name, stored_feature in all_faces:
        similarity = engine.compute_similarity(feature, stored_feature)
        if similarity > max_similarity:
            max_similarity = similarity
            matched_name = name

    # 4. 判断是否匹配（阈值 0.6）
    if max_similarity >= 0.6:
        return {
            "code": 200,
            "name": matched_name,
            "similarity": max_similarity
        }
    else:
        return {
            "code": 404,
            "message": "未匹配到已注册人脸"
        }
```

---

## 4. 完整数据流

### 4.1 人脸识别流程

```
前端/开发板
    ↓ HTTP POST (multipart/form-data)
FastAPI 路由
    ↓ await file.read()
bytes 对象
    ↓ engine.extract_feature(bytes)
Python Wrapper (face_engine.py)
    ↓ ctypes 传递内存指针
C++ 动态库 (libface_engine.so)
    ├─ cv::imdecode (解码)
    ├─ RetinaFace (检测)
    ├─ align_face (对齐)
    ├─ MobileFaceNet (识别)
    └─ return feature[512]
    ↓ 返回特征向量
Python Wrapper
    ↓ .tolist()
FastAPI 路由
    ├─ 查询数据库
    ├─ 计算相似度
    └─ 匹配用户
    ↓ JSON 响应
前端/开发板
```

### 4.2 时间开销（RK3568）

| 阶段 | 耗时 | 说明 |
|------|------|------|
| HTTP 上传 | ~50ms | 取决于图片大小和网络 |
| 图片解码 | ~15ms | OpenCV 解码 JPEG |
| RetinaFace 推理 | ~60ms | NPU 加速 |
| 人脸对齐 | ~5ms | 仿射变换 |
| MobileFaceNet 推理 | ~40ms | NPU 加速 |
| 数据库查询 | ~10ms | 取决于注册人数 |
| **总计** | **~180ms** | 单次识别 |

---

## 5. 实现状态

### ✅ 已完成

- ✅ Python Wrapper 类（`backend/core/face_engine.py`）
- ✅ 5 个 C 函数封装（Create/Init/Extract/Destroy/CosineSimilarity）
- ✅ 单例模式实现
- ✅ 完整的类型注解和文档字符串
- ✅ 错误处理和日志输出

### 🔄 待实现

- ⏳ FastAPI 路由接口（`/api/face/register`, `/api/face/recognize`）
- ⏳ 数据库模型（存储用户名和特征向量）
- ⏳ 开发板后台线程（实时人脸检测）

---

## 6. 注意事项

### 6.1 内存管理

- ✅ **推荐**：使用 `numpy` 管理内存（自动释放）
- ❌ **避免**：手动使用 `ctypes.create_string_buffer`（容易泄漏）

### 6.2 线程安全

- C++ 引擎**不是线程安全的**
- 如果 FastAPI 使用多线程，需要加锁：
  ```python
  import threading

  class FaceEngine:
      def __init__(self):
          self._lock = threading.Lock()

      def extract_feature(self, image_bytes):
          with self._lock:
              # ... 调用 C++ 函数 ...
  ```

### 6.3 错误处理

- 所有 C++ 错误通过返回值传递，**不会抛出 Python 异常**
- Wrapper 层应检查返回值，并在必要时转换为 Python 异常

### 6.4 性能优化

- ✅ 使用单例模式（避免重复加载模型）
- ✅ 使用 `numpy.frombuffer`（零拷贝）
- ✅ 批量识别时复用引擎实例

---

## 7. 参考文档

- C++ API 详细规范：`face_detection/docs/new_docs/format_out_in.md`
- 数据流详解：`face_detection/docs/new_docs/dataflow.md`
- 模型功能说明：`face_detection/docs/new_docs/model_function.md`
